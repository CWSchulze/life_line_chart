...
class ancestor_graph_family():
    ...
    def __init__(self, instances, family_id):
        ...
        self.__instances[('f',self.family_id)].graphical_representations.append(self)
    ...
    def get_spouse(self, individual):
        spouse = self.__instances[('f',self.family_id)].get_spouse(individual.individual_id)
    ...
    def add_visible_children(self, individual):
        if individual.individual_id not in self.visible_children and individual.graphical_representations[0].get_birth_event():
            self.visible_children[individual.individual_id] = (individual.graphical_representations[0].get_birth_event()['ordinal_value'], len(self.visible_children), individual)
