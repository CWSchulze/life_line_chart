...
def estimate_death_date(individual):
    ...
    if individual.events['death_or_burial'] is None and not individual.events['birth_or_christening'] is None:
        ...
        if date > datetime.datetime.now():
            ...
            individual.events['death_or_burial']['ordinal_value'] = datetime.datetime.now().toordinal()
...
def estimate_birth_date(individual, instances):
    ...
    if individual.events['birth_or_christening'] is None:
        ...
        if individual.events['birth_or_christening']:
            date = individual.events['birth_or_christening']['date']
            individual.events['birth_or_christening']['date'] = datetime.datetime(date.year+1, date.month, date.day, 0, 0, 0)
    ...
    if individual.events['birth_or_christening'] is None:
        ...
        if individual.events['birth_or_christening'] and individual.events['birth_or_christening']['date'].year > 25:
            date = individual.events['birth_or_christening']['date']
            individual.events['birth_or_christening']['date'] = datetime.datetime(date.year-25, 1, 1, 0, 0, 0)
...
class BaseIndividual():
    ...
    def _initialize(self):
        ...
        unsorted_marriages = [self._instances[('f',m)] for m in self._marriage_family_ids]
        sorted_pairs = zip([(m.marriage['ordinal_value'], i) if m.marriage else (0, i) for i, m in enumerate(unsorted_marriages)], unsorted_marriages)
    ...
    def _get_birth_label(self):
        ...
        if self.events['birth_or_christening']:
            ...
            if event['comment']:
                string = self.date_label_translation[event['comment']].format(symbol='*', datum= str(event['date'].date().year))
    ...
    def _get_info_text(self):
        ...
        for father, mother in self.get_father_and_mother():
            if father:
                content.append('Vater: {} ({})'.format(self.get_father_and_mother()[0][0].plain_name, self.get_father_and_mother()[0][0].birth_label))
            if mother:
                content.append('Mutter: {} ({})'.format(self.get_father_and_mother()[0][1].plain_name, self.get_father_and_mother()[0][1].birth_label))

        content.append('')
        for marriage in self.marriages:
            spouse = marriage.get_spouse(self.individual_id)
            content.append('Partner: {} ({}), Heirat: {}'.format(spouse.plain_name, spouse.birth_label, marriage.label))
            for index, child in enumerate(marriage.get_sorted_children()):
                content.append(' {}. Kind: {} ({})'.format(index + 1, child.plain_name, child.birth_label))
