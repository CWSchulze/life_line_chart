        ...
        for graphical_individual_representation in self.graphical_individual_representations:
            ...
            for i, value in enumerate(x_pos.values()):
                ...
                if x_index < 0:
                    ...
                    collisions.append(
                        (graphical_individual_representation, None))
                if x_index not in v:
                    v[x_index] = []
                ...
                if i < len(x_pos) - 1:
                    position_to_person_map[x_index].append({
                        'start': list(x_pos.values())[i][0],
                        'end': list(x_pos.values())[i+1][0],
                        'individual': graphical_individual_representation
                    })
                else:
                    position_to_person_map[x_index].append({
                        'start': list(x_pos.values())[i][0],
                        'end': graphical_individual_representation.get_death_event()['ordinal_value'],
                        'individual': graphical_individual_representation
                    })
