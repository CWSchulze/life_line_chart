...
class AncestorGraph(BaseGraph):
    ...
    def select_individuals(self, individual, generations = None, color = None, filter = None):
        ...
        if not individual.has_graphical_representation():
            individual_representation = self._create_individual_graphical_representation(individual)
            
            if individual_representation is None:
                return

            if color is None:
                i = int(hashlib.sha1(" ".join(individual_representation.name).encode('utf8')).hexdigest(), 16) % (10 ** 8)
        ...
        for child_of_family in child_of_families:
            family = self._create_family_graphical_representation(child_of_family)
            ...
            if generations > 0 or generations < 0:
                ...
                if father:
                    self.select_individuals(father, generations - 1, color = individual_representation.color if self._positioning['fathers_have_the_same_color'] else None, filter=filter)
                if mother:
                    self.select_individuals(mother, generations - 1, filter=filter)
    ...
    def place_selected_individuals(self, individual, child_family, spouse_family, child_of_family, x_offset = 0):
        ...
        for local_child_of_family in child_of_families:
            ...
            if father and father.has_graphical_representation():
                ...
                if not father.graphical_representations[0].get_x_position() or local_child_of_family.family_id not in father.graphical_representations[0].get_x_position():
                    father.graphical_representations[0].visual_placement_child = (individual, spouse_family)
                    ...
                    self.place_selected_individuals(father, spouse_family, local_child_of_family, fathers_born_in_family, x_position)
                    #x_position = father.graphical_representations[0].get_x_position()
                    width = father.graphical_representations[0].get_width(spouse_family)
                    if local_child_of_family:
                        local_child_of_family.graphical_representations[0].husb_width = width
                    x_position += width

        children_start_x = x_position
        if individual.graphical_representations[0].get_x_position() is None or not spouse_family is None and spouse_family.family_id not in individual.graphical_representations[0].get_x_position() or False:
            individual.graphical_representations[0].set_x_position(x_position, spouse_family)
            ...
            if child_of_family and not child_of_family.family_id in individual.graphical_representations[0].get_x_position():
                individual.graphical_representations[0].set_x_position(x_position, child_of_family, True)
            x_position += 1
                ...
                if sibling.individual_id == individual.individual_id:
                    ...
                    if not sibling.graphical_representations[0].get_x_position() or not child_of_family.family_id in sibling.graphical_representations[0].get_x_position():
                        if not sibling.graphical_representations[0].visual_placement_child:
                            sibling.graphical_representations[0].visual_placement_child = individual.graphical_representations[0].visual_placement_child
                            sibling.graphical_representations[0].set_x_position(x_position, child_of_family)
        ...
        if child_of_family and child_of_family.has_graphical_representation() and not child_of_family.graphical_representations[0].children_width:
            child_of_family.graphical_representations[0].children_width = x_position - children_start_x
        ...
        for local_child_of_family in child_of_families:
            ...
            if mother and mother.has_graphical_representation():
                ...
                if not mother.graphical_representations[0].get_x_position() or local_child_of_family.family_id not in mother.graphical_representations[0].get_x_position():
                    mother.graphical_representations[0].visual_placement_child = (individual, spouse_family)
                    #mother.graphical_representations[0].visual_placement_child = spouse_family
                    local_child_of_family.graphical_representations[0].visual_placement_child = individual
                    self.place_selected_individuals(mother, spouse_family, local_child_of_family, mothers_born_in_family, x_position)
                    #x_position = mother.graphical_representations[0].get_x_position()
                    width = mother.graphical_representations[0].get_width(spouse_family)
                    if local_child_of_family:
        ...
        if child_of_family:
            if child_of_family.family_id not in individual.graphical_representations[0].widths:
                if child_of_family and child_of_family.has_graphical_representation() and len(child_of_family.graphical_representations[0].visible_children) > 1:
                    individual.graphical_representations[0].widths[child_of_family.family_id] = max(0, individual.graphical_representations[0].x_end - individual.graphical_representations[0].x_start)
                    individual.graphical_representations[0].range[child_of_family.family_id] = (individual.graphical_representations[0].x_start, individual.graphical_representations[0].x_end)
                else:
                    individual.graphical_representations[0].widths[child_of_family.family_id] = max(0, individual.graphical_representations[0].x_end - individual.graphical_representations[0].x_start)
                    individual.graphical_representations[0].range[child_of_family.family_id] = (individual.graphical_representations[0].x_start, individual.graphical_representations[0].x_end)
                    #print("as")
        ...
        if not child_of_family or not child_of_family.graphical_representations:
            ...
            for _, _, sibling in sorted(child_of_family.graphical_representations[0].visible_children.values()):
                if child_family:
                    sibling.graphical_representations[0].widths[child_family.family_id] = max(0, individual.graphical_representations[0].x_end - individual.graphical_representations[0].x_start)
                    sibling.graphical_representations[0].range[child_family.family_id] = (individual.graphical_representations[0].x_start, individual.graphical_representations[0].x_end)
                else:
                    sibling.graphical_representations[0].widths[child_family] = max(0, individual.graphical_representations[0].x_end - individual.graphical_representations[0].x_start)
                    sibling.graphical_representations[0].range[child_family] = (individual.graphical_representations[0].x_start, individual.graphical_representations[0].x_end)
        min_ordinal = 9e99
        max_ordinal = -9e99
    ...
    def _compress_graph_ancestor_graph(self, graphical_family_representation):
        ...
        if graphical_family_representation is None:
            return
        if not graphical_family_representation.husb is None and graphical_family_representation.husb.has_graphical_representation():
            individuals.append((graphical_family_representation.husb.graphical_representations[0].get_x_position()[graphical_family_representation.family_id][1], graphical_family_representation.husb))
        if not graphical_family_representation.wife is None and graphical_family_representation.wife.has_graphical_representation():
            individuals.append((graphical_family_representation.wife.graphical_representations[0].get_x_position()[graphical_family_representation.family_id][1], graphical_family_representation.wife))
        for index, (_, individual) in enumerate(sorted(individuals)):
            ...
            for cof in cofs:
                if cof.has_graphical_representation():
                    self._compress_graph_ancestor_graph(cof.graphical_representations[0])
        for index, (_, individual) in enumerate(sorted(individuals)):
            ...
            try:
                while i < 50000:
                    self._move_individual_and_ancestors(individual, individual.get_child_of_family()[0], direction_factor*1)
                    self._check_compressed_x_position(True)
                    i += 1
            except ValueError:
                #print("   collision of " + " and ".join([" ".join(a.name) for a in e.args]))
                self._move_individual_and_ancestors(individual, individual.graphical_representations[0].visible_parent_family, -direction_factor*1)
            if i != 0:
                print ('moved ' + ' '.join(individual.name) + ' by ' + str(i * direction_factor * 1))
    ...
    def modify_layout(self, root_individual_id):
        ...
        if self._positioning['flip_to_optimize']:
            ...
            #candidantes = set()  
            items = list(reversed(sorted([(child.graphical_representations[0].get_birth_event()['ordinal_value'], index, child) for index, child in enumerate(candidantes)])))
            failed = []
            for ov, index, child in items:
                c_pos = list(child.graphical_representations[0].get_x_position().values())[1:]
                for x_pos in c_pos:
                    if x_pos[2] is None:
                        continue
                    #family_id = key2[2]
                    ...
                    if len(failed) > 0:
                        print("failed flipping "+ str((x_pos[2].family_id, ov)))
                        break
            ...
            print(f"flipping reduced the cross connections by {width - old_width} (i.e. from {old_width} to {width})")
        ...
        if self._positioning['compress']:
            ...
            self._compress_graph_ancestor_graph(self._instances[('i',root_individual_id)].graphical_representations[0].visible_parent_family)
            ...
            _, _, _, self.position_to_person_map = self._check_compressed_x_position(False)
    ...
    def define_svg_items(self):
        ...
        # setup grid
        max_y = max(self._map_y_position(self.min_ordinal), self._map_y_position(self.max_ordinal))
        min_y = min(self._map_y_position(self.min_ordinal), self._map_y_position(self.max_ordinal))
        min_x_index = self.min_x_index
        max_x_index = self.max_x_index
        for index in range(600):
            year  = 1000 + 2*index 
            year_pos = self._map_y_position(datetime.date(year,1,1).toordinal())
        ...
        for index in range(1000):
            ...
            if x_position > min_x_index and x_position < max_x_index :
                svg_path = Path(Line(x_position + min_y*1j, x_position + max_y*1j ))
                if index % 20 == 0:
        ...
        for graphical_individual_representation in self.graphical_individual_representations:
            ...
            x_pos_list = sorted([(ov, pos, index, family_id, flag) for index, (family_id, (ov, pos, f, flag)) in enumerate(x_pos.items())])
            ...
            if graphical_individual_representation.get_marriages():
                for graphical_representation_marriage_family in graphical_individual_representation.get_marriages():
                    if graphical_representation_marriage_family.marriage is None:
                        continue
                    spouse_representation = graphical_representation_marriage_family.get_spouse(graphical_individual_representation.individual)
                    marriage_x_index = x_pos[graphical_representation_marriage_family.family_id][1]
                    new_x_position_after_marriage.append(self._map_x_position(marriage_x_index))
                    ...
                    if spouse_representation and spouse_representation.get_x_position() and graphical_representation_marriage_family.marriage:
                        ...
                        spouse_x_index = spouse_representation.get_x_position()[graphical_representation_marriage_family.family_id][1]
                        ...
                        marriage_ring_indices.append((spouse_x_index + marriage_x_index)/2.)
                    else:
                        ...
                        for _, (_, _, visible_child) in graphical_representation_marriage_family.visible_children.items():
                            try:
                                child_x_indices.append(visible_child.graphical_representations[0].get_x_position()[graphical_representation_marriage_family.family_id][1])
                            except:
                                print('something went wrong with ' + "".join(visible_child.name) + ". The position family 0 is not equal to the placement...")
                        if len(child_x_indices) > 0:
                            # calculate the middle over the children
                            marriage_ring_positions.append(self._map_position(
                                sum(child_x_indices)/len(child_x_indices),
                                graphical_representation_marriage_family.marriage['ordinal_value']))
                            marriage_ring_indices.append(sum(child_x_indices)/len(child_x_indices))
                        else:
                            ...
                            marriage_ring_positions.append(self._map_position(
                                ...
                                graphical_representation_marriage_family.marriage['ordinal_value']))
                            marriage_ring_indices.append(x_pos[graphical_representation_marriage_family.family_id][1])
                        
                    marriage_y_positions.append(self._map_y_position(graphical_representation_marriage_family.marriage['ordinal_value']))
                    marriage_ordinals.append(graphical_representation_marriage_family.marriage['ordinal_value'])
                    marriage_labels.append(str(graphical_representation_marriage_family.label))
                    

            # generate event node information
            knots = []
            _birth_original_location = (x_pos_list[0][1], birth_event['ordinal_value'])
            _death_original_location = (x_pos_list[-1][1], death_event['ordinal_value'])
            ...
            for index, (marriage_ring_index, marriage_ordinal, new_x_index_after_marriage, label) in enumerate(zip(marriage_ring_indices, marriage_ordinals, new_x_indices_after_marriage, marriage_labels)):
                if not self._formatting['no_ring']:
                    ring_position  = self._map_position(marriage_ring_index, marriage_ordinal)
                    images.append(
                        {
                            'type' : 'image',
                            'config' : {
                                'insert' : (
                                    ring_position[0] - self._formatting['relative_line_thickness']*self._formatting['vertical_step_size']*1,
                ...
                if self._formatting['marriage_label_active']:
                    dy_line = self._inverse_y_position(self._formatting['relative_line_thickness']*self._formatting['vertical_step_size']) - self._inverse_y_position(0)
                    images.append(
                        {
                            'type':'text',
                            'config':{
                ...
                if len(marriage_ordinals) > index + 1:
                    knots.append((new_x_index_after_marriage, marriage_ordinals[index]/2+marriage_ordinals[index+1]/2)) # zwischenpunkt zur ursprungsposition
            ...
            def marriage_bezier(data, knots, flip = False):
                ...
                if len(knots) == 2:
                    data.append(
                        ({'type':'Line', 'arguments':(
                            coordinate_transformation(knots[0][0], knots[0][1]),
                            coordinate_transformation(knots[0+1][0], knots[0+1][1]),
                        )},
                        ...
                        (_birth_position[1], self._map_y_position(self._formatting['fade_individual_color_black_age']*365+birth_event['ordinal_value']))
                        )
                    )
                else:
                        ...
                        if (index + t) % 2 == 0:
                            data.append(
                                ({'type':'CubicBezier', 'arguments':(
                                    coordinate_transformation(knots[index][0], knots[index][1]),
                                    coordinate_transformation(knots[index][0], knots[index+1][1]),
                                    coordinate_transformation(knots[index][0], knots[index+1][1]),
                                    coordinate_transformation(knots[index+1][0], knots[index+1][1]),
                                )},
                            ...
                            data.append(
                                ({'type':'CubicBezier', 'arguments':(
                                    coordinate_transformation(knots[index][0], knots[index][1]),
                                    coordinate_transformation(knots[index+1][0], knots[index][1]),
                                    coordinate_transformation(knots[index+1][0], knots[index][1]),
                                    coordinate_transformation(knots[index+1][0], knots[index+1][1]),
                                )},
                                #((knots[index][1]-_birth_position[1])/(self._map_y_position(self._formatting['fade_individual_color_black_age']*365+birth_event['ordinal_value'])-_birth_position[1]), (knots[index+1][1]-_birth_position[1])/(self._map_y_position(self._formatting['fade_individual_color_black_age']*365+birth_event['ordinal_value'])-_birth_position[1])),
                    ...
                    graphical_individual_representation.items.append(
                        {
                            ...
                            'spans':[
                                (individual_name[0], {'dx':self._formatting['birth_label_letter_x_offset']}),
                                (individual_name[1], {'style':'font-weight: bold'}),
                                (birth_label, {})
                                ],
    ...
    def paint_and_save(self, individual_id, filename = None):
        ...
        if filename is None:
            filename = 'ancestors_of_' + "".join(self._instances[('i',individual_id)].name).replace(' ', '')
            if self._positioning['flip_to_optimize']:
                filename += '_flipped'
            if self._positioning['compress']:
        ...
        sorted_individuals = [(gr.get_birth_event()['ordinal_value'], index, gr)  for index, gr in enumerate(self.graphical_individual_representations)]
        ...
        for item in additional_items + sorted_individual_items:
                if item['type'] == 'text':
                    ...
                    if self._formatting['fade_individual_color'] and 'color_pos' in item:
                        ...
                        fill.add_stop_color(0, "rgb({},{},{})".format(*item['color']))
                        ...
                        svg_document.add(svg_document.path(d=svg_path.d(), stroke=fill.get_paint_server(default='currentColor'), fill = 'none', stroke_width=item['stroke_width']))
                    else:
                    #arguments['fill'] = fill
                    #graphical_individual_representation.color
                        svg_document.add(svg_document.path(d=svg_path.d(), stroke="rgb({},{},{})".format(*item['color']), fill = 'none', stroke_width=item['stroke_width']))
                    ...
                    svg_document.add(y)
                    #x = svg_document.add(svg_text)
                    x = svg_document.add(svgwrite.text.Text('', dy = [args_text['dy']], style = args_text['style']))
                    ...
                    if key in image_defs:
                        ...
                        this_def['image_data'] = open(item['filename'],'rb').read()
                        this_def['encoded'] = base64.b64encode(this_def['image_data']).decode()
                        this_def['pngdata'] = 'data:image/png;base64,{}'.format(this_def['encoded'])
                        image = Image.open(item['filename'])
                        this_def['size'] = image.size
                        this_def['image_content'] = svg_document.image(href=(this_def['pngdata']), preserveAspectRatio='xMidYMid', size=(1,1))
                        this_def['image_def'] = svg_document.defs.add(this_def['image_content'])
                    ...
                    svg_document.add(svg_document.use(this_def['image_def'].get_iri(), transform=f"translate({pos_x-width/2*0},{pos_y - height/2*0}) scale({width},{height})"))
                    pass
