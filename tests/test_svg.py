from life_line_chart import AncestorGraph, DescendantGraph, get_gedcom_instance_container
import pytest
import os
try:
...
def test_generate_svg_file_ancestor_and_children():
    ...
    i = 0
    while i < 4:
        for cof in graph._instances[('i', individual2_id)].get_child_of_family():
...
def test_generate_svg_update_interface():
    ...
    # select individual to show
    #graph.set_positioning({'compress':True, 'flip_to_optimize':True})
    ...
    graph.paint_and_save('@I25@', os.path.join(
        os.path.dirname(__file__), 'output', 'test_svg_configuration.svg'))


def test_generate_svg_update_interface_descendant():
    graph = DescendantGraph(instance_container=lambda: get_gedcom_instance_container(
        os.path.join(os.path.dirname(__file__), 'autogenerated.ged')))
